...
public class Echoer extends Thread{
    ...
    private boolean cancelVisit() throws IOException{
        ...
        name = input.readLine();
        if(name.length() < 2){
            output.println("Name is incomplete. Try again typing 2.");
            return false;
        }
        if(Schedule.getInstance().cancelDate(intHour,name)){
//            notify = new Notify(this,"## " + intHour + ":00 is now available. ##", "There has been a change.", 2);
            output.println("You canceled the appointment at " + intHour + ":00.");
            HairSalonServer.notifyAllClients(this,"## " + intHour + ":00 is now available. ##");
        } else {
            output.println("Failed to cancel your appointment. Type 2 to try again.");
            return false;
        }
        intHour = -1;
        return true;
    ...
    private int checkHour(String stringHour) {
        ...
        do {
            try {
                if(i > 0) {
                    output.println("Input number between (included) 10 and 18 :");
                    stringHour = input.readLine();
                }
    ...
    private static String printMenu(){
        String s = "What do you want to do?\n" +
                "1 - Book visit.\n" +
                "2 - Cancel visit.\n" +
                "3 - Print all available dates.\n" +
                "Write 'exit' if you want to close connection with hairdresser.\n";
        return s;
    }
